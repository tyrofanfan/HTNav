...
from gsamllavanav.dataset.generate import convert_trajectory_to_shortest_path
from gsamllavanav.mapdata import GROUND_LEVEL
from gsamllavanav.space import view_area_corners
...
def eval_goal_predictor(
    ...
    for eps in episodes:
        ...
        sp_episode = convert_trajectory_to_shortest_path(eps)
        optimal_length = calculate_path_length(sp_episode.teacher_trajectory)
...
def run_episodes_batch(
    ...
    print("Loaded maps:", list(cropclient._rgb_cache.keys()))
    ...
    for episodes_batch in tqdm(dataloader, desc='eval episodes', unit='batch', colour='#88dd88', position=1, disable=True):

        batch_size = len(episodes_batch)
        poses = [eps.start_pose for eps in episodes_batch]
        dones = np.zeros(batch_size, dtype=bool)
        ema_goal_xys = [None] * batch_size
        alpha = 0.8
        nav_maps = [
            LandmarkNavMap(
        ...
        for t in trange(args.eval_max_timestep, desc='eval timestep', unit='step', colour='#66aa66', position=2, leave=False, disable=True):
            ...
            for eps, pose, noisy_pose, nav_map, done in tqdm(zip(episodes_batch, poses, noisy_poses, nav_maps, dones), desc='updating maps', unit='map', colour='#448844', position=3, leave=False, disable=True):
                if not done:
            ...
            outputs = predictor(maps, rgbs, normalized_depths, flip_depth=True)
            ...
            control_goal_xys = []
            for i, (done, raw_xy) in enumerate(zip(dones, pred_goal_xys)):
                if done:
                    control_goal_xys.append(raw_xy)
                    continue
                if ema_goal_xys[i] is None:
                    ema_goal_xys[i] = raw_xy
                else:
                    ema_goal_xys[i] = Point2D(
                        alpha * ema_goal_xys[i].x + (1 - alpha) * raw_xy.x,
                        alpha * ema_goal_xys[i].y + (1 - alpha) * raw_xy.y,
                    )
                control_goal_xys.append(ema_goal_xys[i])
            ...
            poses = [
                move(pose, xy, args.move_iteration, noisy_pose) if not done else pose
                for pose, noisy_pose, xy, done in zip(poses, noisy_poses, control_goal_xys, dones)
            ]

    return (
        dict(pose_logs), 
        dict(pred_goal_logs), 
        dict(pred_progress_logs)
    )
...
def get_landmark_xy(episode, landmark_names):
    return [
        [episode.map_landmarks[name].x, episode.map_landmarks[name].y]
        for name in landmark_names if name in episode.map_landmarks
    ]
